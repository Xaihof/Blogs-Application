...
import com.example.blogx.R
import com.example.blogx.ReadMoreActivity
import com.example.blogx.databinding.BlogItemBinding
...
class BlogAdapter(private val items: MutableList<BlogItemModel>) :
    ...
    inner class BLogViewHolder(private val binding: BlogItemBinding) :
        ...
        fun bind(blogItemModel: BlogItemModel) {
            ...
            // Set the initial icon based in the saved status.
    ...
    private fun handleLikeButtonClicked(
        ...
        val userReference = databaseReference.child("users").child(currentUser!!.uid)
        val postLikesReference = databaseReference.child("blogs").child(postId).child("likes")

        // User has already liked the post, so unlike it.
        postLikesReference.child(currentUser.uid)
            .addListenerForSingleValueEvent(object : ValueEventListener {
                override fun onDataChange(snapshot: DataSnapshot) {
                    if (snapshot.exists()) {
                        userReference.child("likes").child(postId).removeValue()
                            .addOnSuccessListener {
                                postLikesReference.child(currentUser.uid).removeValue()
                        ...
                        userReference.child("likes").child(postId).setValue(true)
                            .addOnSuccessListener {
                                postLikesReference.child(currentUser.uid).setValue(true)
        ...
        userReference.child("saveBlogPosts").child(postId)
            .addListenerForSingleValueEvent(object : ValueEventListener {
                override fun onDataChange(snapshot: DataSnapshot) {
                    ...
                    if (snapshot.exists()) {
                        // The blog is currently saved, so unsave it.
